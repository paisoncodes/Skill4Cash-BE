...
class CustomerProfileSetUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        validated_data["user"] = user = User.objects.filter(
...
class ServiceProviderProfileSetUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        business_profile = {}
        business_profile["user"] = validated_data["user"] = user = User.objects.filter(
            id=validated_data["user"]
        ).first()
        ...
        validated_data["state"] = State.objects.get(
            state__iexact=validated_data["state"]
        )
        ...
        profile, _ = UserProfile.objects.get_or_create(**validated_data)

        business, _ = BusinessProfile.objects.get_or_create(**business_profile)
...
class ImageSerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if image_fields:
            image_update_dict = {
                _: serializers.ImageField(required=False, write_only=True)
                for _ in image_fields
            }
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserProfile
        exclude = ("user", "id", "date_created", "last_update")
