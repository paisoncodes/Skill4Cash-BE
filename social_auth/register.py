...
def register_social_user(
    provider, email, first_name, last_name, username, role, state, city
):
    ...
    if filtered_user_by_email.exists():
        ...
        if provider == filtered_user_by_email[0].auth_provider:
            ...
            registered_user = create_token(password=settings.SOCIAL_SECRET, email=email)
            if "error" not in registered_user.keys():
        ...
        registered_user = create_token(
            password=settings.SOCIAL_SECRET, email=email, phone_number=None
        )
